...
21. 任務已有核准樣本時，一般照片必須逐張對應樣本照片且不得修改樣本說明與順序；尚無核准樣本時，新增照片的 `PhotoDescription` 與 `SortOrder` 必填；0 張照片完成任務時 `ExecutionNote` 必填。
22. 原始匯入 Excel 必須永久保存且不得被覆蓋。
23. 作廢或被取代的匯入批次不得參與正式庫存計算。
24. 型號無法依規則轉為有效 ProductId 時，整批匯入失敗。
25. `OpeningInventoryProductExclusion.ScopeType` 只能是 `ALL` 或 `DEALER`。
26. `ScopeType = ALL` 時 `DealerId` 必須為 NULL；`ScopeType = DEALER` 時 `DealerId` 必填。
