...
1. 從目標日期所屬月份的有效正式期初庫存批次，取得 `OpeningQuantity`。非零的客戶／商品配對進入 PSI；零值、空值，以及期初匯入標記「不計」而建立有效 PSI 排除規則的商品不因期初資料而顯示。
2. 加總該月月初至報表截止日之間有效的 `SellInTransaction.Quantity`（正數為 `Sell In`、負數為退貨）。若商品在月中上市、當月期初沒有該商品，只要有有效 Sell In 或退貨，仍建立 PSI 商品列，並以 `OpeningQuantity = 0` 作為計算起點。若期初為零但當月有有效 Sale In，同樣必須顯示。
3. 加總並扣除該月月初至報表截止日之間有效巡店中的 `StoreVisitProductDetail.SellOutQuantity`。僅有 Sell Out、沒有非零期初且沒有有效 Sell In／退貨時，不單獨建立 PSI 商品列。
4. 陳列數量取報表截止時間以前，該經銷商最後一次 `RecordStatus = ACTIVE` 的巡店紀錄所含商品明細。該次巡店可早於 PSI 月份；不跨巡店紀錄逐商品尋找更舊數值，也不將多次巡店累加。最後一次有效巡店未包含該商品或陳列為空時，陳列留白。
...
## 2026-09-15｜月度型號陳列照片與陳列繼承

- 陳列照片整合在實銷／陳列巡店流程，但不使用公司任務的 `VisitTaskPhoto`。
- 照片有效鍵為 `DataMonth＋DealerId＋ProductId`：同一經銷商同一型號每月只需一張有效照片；不同有陳列型號各自需要照片。
- 實銷每次巡店重新填寫。陳列數量只在同月份內，從該經銷商／型號最近一次有效巡店繼承；正式送出時仍完整寫入本次 `StoreVisitProductDetail`，維持 PSI 的最後巡店快照語意。
- 本月已有該型號照片時不重拍；新增陳列且本月尚無照片時提示立即拍照或本次跳過。跳過不阻擋 Sale Out，後續巡店再次提示待補。
- 陳列照片沿用公司任務照片的手機拍照 UI：即時取景、預覽與裁切結果均為 1:1，使用者可拖曳取景及縮放，確認後輸出 1200 × 1200 px 原圖與同內容正方形縮圖並上傳。
- `DealerProductDisplayPhoto` 只保存月份、關聯、中繼資料、SHA-256、正式機相對路徑及保留狀態；原圖與縮圖二進位均不得寫入 SQL Server。
- 同月重拍時新增紀錄，舊紀錄改為 `SUPERSEDED`；PSI 只採 `ACTIVE` 照片並顯示 `陳列數量 📷`。
- 原圖目標保存三年；縮圖可長期保存。原圖清除後寫入 `OriginalDeletedAt`，不得刪除縮圖或資料庫稽核紀錄。
- 正式照片固定存放於正式機 `uploads/display_photos/YYYY/MM/DealerId/`；測試機不保存或同步正式照片。資料庫與照片目錄必須成對備份。
