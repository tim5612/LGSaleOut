...
這三份檔案都使用 `USE [LGSaleOut]`。每次執行前，在 SSMS 確認目前連線是要重置的那台 SQL Server；若資料庫名稱不同，先修改 USE。三台應先同步最新程式與資料表 migration，不要重跑 CreateDB.sql 來清資料。
...
- 「初始管理員」是本次建立的員工名稱，不代表新增了一套管理員角色權限模型；沿用現有員工帳戶權限。
