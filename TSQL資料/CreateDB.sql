/*
    LGSaleOut - database creation, login and schema script
    ...
    3. Run this script in SSMS with a sysadmin account.
    4. Development login: Tim / 561202; Tim is added to db_owner.
*/
...
ALTER ROLE [db_owner] ADD MEMBER [Tim];
GO
...
(
    ...
    CreatedAt            datetime2(0) NOT NULL
        CONSTRAINT DF_OpeningInventoryProductExclusion_CreatedAt DEFAULT (sysdatetime()),
    CreatedByEmployeeId  bigint NOT NULL,

    CONSTRAINT PK_OpeningInventoryProductExclusion
        PRIMARY KEY CLUSTERED (ExclusionId),
    CONSTRAINT FK_OpeningInventoryProductExclusion_Product
        FOREIGN KEY (ProductId) REFERENCES dbo.Product (ProductId),
    CONSTRAINT FK_OpeningInventoryProductExclusion_Dealer
        FOREIGN KEY (DealerId) REFERENCES dbo.Dealer (DealerId),
    CONSTRAINT FK_OpeningInventoryProductExclusion_CreatedByEmployee
        FOREIGN KEY (CreatedByEmployeeId) REFERENCES dbo.Employee (EmployeeId),
    CONSTRAINT CK_OpeningInventoryProductExclusion_Scope
        CHECK
        (
            (ScopeType = 'ALL' AND DealerId IS NULL)
            OR (ScopeType = 'DEALER' AND DealerId IS NOT NULL)
        ),
    CONSTRAINT CK_OpeningInventoryProductExclusion_FromMonth
        CHECK
        (
            EffectiveFromMonth NOT LIKE '%[^0-9]%'
            AND SUBSTRING(EffectiveFromMonth, 5, 2) BETWEEN '01' AND '12'
        ),
    CONSTRAINT CK_OpeningInventoryProductExclusion_ToMonth
        CHECK
        (
            EffectiveToMonth IS NULL
            OR
            (
                EffectiveToMonth NOT LIKE '%[^0-9]%'
                AND SUBSTRING(EffectiveToMonth, 5, 2) BETWEEN '01' AND '12'
                AND EffectiveToMonth >= EffectiveFromMonth
            )
        )
);
GO

CREATE UNIQUE INDEX UX_OpeningInventoryProductExclusion_All
    ON dbo.OpeningInventoryProductExclusion (ProductId, EffectiveFromMonth)
    WHERE ScopeType = 'ALL';
GO

CREATE UNIQUE INDEX UX_OpeningInventoryProductExclusion_Dealer
    ON dbo.OpeningInventoryProductExclusion (DealerId, ProductId, EffectiveFromMonth)
    WHERE ScopeType = 'DEALER';
GO

CREATE INDEX IX_OpeningInventoryProductExclusion_EffectivePeriod
    ON dbo.OpeningInventoryProductExclusion
       (ProductId, DealerId, EffectiveFromMonth, EffectiveToMonth);
GO
...
(
    ...
    CONSTRAINT CK_StoreVisitProductDetail_SellOutPair
        ...
        (
            ...
            OR (SellOutQuantity IS NOT NULL AND SellOutDate IS NOT NULL)
        ),
    CONSTRAINT CK_StoreVisitProductDetail_HasValue
        CHECK (SellOutQuantity IS NOT NULL OR DisplayQuantity IS NOT NULL)
);
GO
...
(
    ...
    UploadedAt              datetime2(0) NOT NULL CONSTRAINT DF_DealerProductDisplayPhoto_UploadedAt DEFAULT (sysdatetime()),
    UploadedByUserAccountId bigint NOT NULL,
    RecordStatus            varchar(20) NOT NULL CONSTRAINT DF_DealerProductDisplayPhoto_Status DEFAULT ('ACTIVE'),
...
(
    ...
    CONSTRAINT CK_VisitTaskExecution_Submission
        CHECK (SubmittedAt IS NULL OR CompletedByEmployeeId IS NOT NULL)
);
GO
...
/* =========================================================
   7. Verification (the ERD currently defines 20 tables)
   ========================================================= */
