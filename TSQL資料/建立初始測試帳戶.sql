/* SSMS 手動執行。先完成 EraseDummy.sql；只建立一位初始員工與帳戶。
...
BEGIN TRY
    ...
    SELECT DB_NAME() AS DatabaseName, @EmployeeNo AS EmployeeNo, @EmployeeName AS EmployeeName,
           @EmployeeId AS EmployeeId, UserAccountId,
           N'初始帳戶已建立；請執行「產生初始Passkey註冊連結.sql」。' AS NextStep
    FROM dbo.UserAccount WHERE EmployeeId=@EmployeeId;
END TRY
BEGIN CATCH
